...
// void clean_model(struct convolutional_network &net)
// {
//     // (!) Memory Cleanup
//     // net.Filters.clear();
...
//     std::vector<std::vector<std::vector<std::vector<F>>>> X;
//     std::vector<F> L;

//     // 转换 MNIST 数据集
//     convert_MNIST(dataset, X, L);

//         // 打印 dataset 中的图片数量和标签数量
//     std::cout << "Number of images in dataset: " << dataset.training_images.size() << std::endl;
//     std::cout << "Number of labels in dataset: " << dataset.training_labels.size() << std::endl;

//     // 打印 X 中的图片数量和 L 中的标签数量
//     std::cout << "Number of images in X: " << X[0][0].size() << std::endl;
//     std::cout << "Number of labels in L: " << L.size() << std::endl;

//     // 打印 dataset 中的第一个图像和标签信息
//     std::cout << "First image in dataset (raw pixel values):" << std::endl;
//     const auto& first_image = dataset.training_images[0];
//     for (size_t h = 0; h < 28; ++h) {
//         for (size_t w = 0; w < 28; ++w) {
//             std::cout << static_cast<int>(first_image[h * 28 + w]) << " ";  // 打印像素值
//         }
//         std::cout << std::endl;
//     }
//     std::cout << "First label in dataset: " << static_cast<int>(dataset.training_labels[0]) << std::endl;

//     // 打印 X 中的第一个图像信息
//     std::cout << "\nFirst image in X (converted):" << std::endl;
//     for (size_t h = 0; h < 28; ++h) {
//         for (size_t w = 0; w < 28; ++w) {
//             std::cout << X[0][0][h][w].real << " ";  // 打印转换后的像素值
//         }
//         std::cout << std::endl;
//     }

//     // 打印 L 中的第一个标签信息
//     std::cout << "\nFirst label in L (converted): " << L[0].real << std::endl;
