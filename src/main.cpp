...
struct proof prove_ifft_matrix(vector<vector<F>> M, vector<F> r, F previous_sum)
{
    ...
    if (previous_sum != Pr.q_poly[0].eval(0) + Pr.q_poly[0].eval(1))
    {
        ...
        throw std::runtime_error("Error in prove");
    }
...
void clear_network(struct convolutional_network &net)
{
    net.Filters.clear();
...
ParsedArgs parse_arguments(int argc, char *argv[])
{
    if (argc < 6)
    {
        cerr << "Usage: " << argv[0] << " <Model> <Batch> <Channels> <Levels> <PC_scheme>" << endl;
        exit(EXIT_FAILURE);
    }
    ...
    {
        cerr << "Invalid Neural network" << endl;
        exit(EXIT_FAILURE);
    }

    args.Batch = atoi(argv[2]);
    args.channels = atoi(argv[3]);
    args.levels = atoi(argv[4]);
    args.PC_scheme = atoi(argv[5]);

    return args;
}
void generate_witness(convolutional_network &net, vector<F> &witness, vector<F> &new_model)
{
...
void commit_witness(
    const vector<F> &witness,
    vector<vector<F>> &witness_matrix,
    commitment &comm)
{
    poly_commit(witness, witness_matrix, comm, levels);
}

vector<F> generate_aggregation_proof(
    const vector<vector<F>> &witness_matrix,
    const vector<commitment> &comm,
    const ParsedArgs &args)
{
    ...
    return proof_witness;
}

vector<proof> verify_proof_chain(const vector<F> &proof_witness)
{
    vector<proof> u_proof_temp, u_proof;

    vector<F> aggregation_hashes = x_transcript;
    x_transcript.clear();

    u_proof_temp.push_back(verify_proof(Transcript));
    vector<proof> temp_P = mimc_sumcheck(proof_witness);
...
struct convolutional_network deep_copy_network(const struct convolutional_network &net)
{
    struct convolutional_network new_net;

    // 直接复制简单类型成员
    new_net.final_out = net.final_out;
    new_net.final_w = net.final_w;
    new_net.Batch_size = net.Batch_size;
    new_net.flatten_n = net.flatten_n;
    new_net.convolution_pooling = net.convolution_pooling;
    new_net.w = net.w;
    new_net.der_dim = net.der_dim;

    // 深拷贝 flatten_input
    new_net.flatten_input = net.flatten_input;

    // 深拷贝 Filters
    new_net.Filters = net.Filters;

    // 深拷贝 Rotated_Filters
    new_net.Rotated_Filters = net.Rotated_Filters;

    // 深拷贝 Weights
    new_net.Weights = net.Weights;

    // 深拷贝 avg_layers
    new_net.avg_layers = net.avg_layers;

    // 深拷贝 avg_backprop
    new_net.avg_backprop = net.avg_backprop;

    // 深拷贝 der
    new_net.der = net.der;

    // 深拷贝 convolutions
    new_net.convolutions = net.convolutions;

    // 深拷贝 convolutions_backprop
    new_net.convolutions_backprop = net.convolutions_backprop;

    // 深拷贝 relus
    new_net.relus = net.relus;

    // 深拷贝 fully_connected
    new_net.fully_connected = net.fully_connected;

    // 深拷贝 fully_connected_backprop
    new_net.fully_connected_backprop = net.fully_connected_backprop;

    // 深拷贝 relus_backprop
    new_net.relus_backprop = net.relus_backprop;
...
int main(int argc, char *argv[])
{
    ...
    levels = 1;
    int first_edit = 1;
    vector<struct convolutional_network> network_backups;

    ParsedArgs args = parse_arguments(argc, argv);

    PC_scheme = args.PC_scheme;
    Commitment_hash = 1;
    levels = args.levels;

    // 初始化网络
    struct convolutional_network net = init_network(args.model, args.Batch, args.channels);

    // 训练参数
    int num_epochs = 3; // 用户输入的训练轮数
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch)
    {
        try
        {
            ...
            clock_t start, end;
            net = feed_forward(X, net, args.channels);
            ...
            // 生成和提交 witness
            vector<F> witness, new_model;
            vector<vector<F>> witness_matrix;
            vector<commitment> comm(2);

            printf("Generating witness...\n");
            proving_time = 0.0;
            generate_witness(net, witness, new_model);
            commit_witness(witness, witness_matrix, comm[0]);

            // 打印 Witness 和 Commit 信息
            printf("Witness size: %d\n", witness.size());
            printf("Commit size: %d, Commit time: %lf\n", witness.size(), new_model.size(), proving_time);

            // Prove feedforward and backpropagation
            printf("Proving feedforward and backpropagation...\n");
            clock_t wc1, wc2;
            wc1 = clock();
            ...
            net = init_network(args.model, args.Batch, args.channels);
        }
